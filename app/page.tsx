...
type Peer = { id: string; name: string; kind: 'phone' | 'computer' };
type Transfer = { id: string; sender: Peer; receiver: Peer; kind: 'files' | 'text'; files: { name: string; size: number }[]; text?: string; total: number; bytes: number; status: string; message?: string; created: number };
...
export default function Home() {
  ...
  const socket = useRef<WebSocket | null>(null), token = useRef('');
  ...
  function connect(joinCode: string, deviceName: string) {
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'joined') { joined = true; token.current = msg.token; setSelf(msg.self); setConnected(true); setConnecting(false); sessionStorage.setItem('nearshare-code', joinCode); localStorage.setItem('nearshare-name', deviceName); return; }
      ...
      if (msg.type === 'upload') {
        ...
        if (!file) { ws.send(JSON.stringify({ type: 'cancel', id: msg.id })); setNotice('File unavailable. Please send it again.'); return; }
  ...
  const joinUrl = `${selectedUrl || location.origin}/#code=${code}`;
  ...
  async function accept(t: Transfer) {
    setAccepting(true);
    try { const response = await fetch(`/api/transfers/${t.id}/accept`, { method: 'POST', headers: { Authorization: `Bearer ${token.current}` } }); const result = await response.json(); if (!response.ok) throw new Error(result.error);
  ...
  return <div className="app-shell">
    ...
    <Dialog open={shareOpen} onOpenChange={setShareOpen}><DialogContent className="app-dialog"><DialogTitle>Connect another device</DialogTitle><DialogDescription>Connect to the same Wi-Fi, then scan this code with your phone’s camera.</DialogDescription><div className="qr-box"><QRCodeSVG value={joinUrl} size={190} level="M" marginSize={2} /></div>{urls.length > 1 && <div className="address-choices"><span>Choose this computer’s Wi-Fi address:</span>{urls.map((url) => <button key={url} className={`button secondary ${url === selectedUrl ? 'chosen-address' : ''}`} onClick={() => setSelectedUrl(url)}>{url}</button>)}</div>}<div className="join-details"><span>Or open this address</span><code>{selectedUrl || location.origin}</code><span>Room code</span><strong className="room-code">{code}</strong></div><button className="button primary wide" onClick={() => copy(joinUrl)}><Link size={17} />Copy invitation link</button><p className="dialog-note">Keep both pages open while sharing. Share this code only with people you trust.</p></DialogContent></Dialog>
