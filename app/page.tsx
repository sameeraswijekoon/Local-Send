...
type Transfer = { id: string; sender: Peer; receiver: Peer; kind: 'files' | 'text'; files: { name: string; size: number }[]; text?: string; total: number; bytes: number; status: string; message?: string; created: number; downloads?: { name: string; url: string }[]; speed?: number };
...
export default function Home() {
  ...
  const speedSamples = useRef(new Map<string, { bytes: number; at: number }>());
  ...
  function handleMessage(event: { data: string }) {
    ...
    if (msg.type === 'transfer') {
      ...
      const previous = speedSamples.current.get(t.id), now = performance.now();
      const speed = t.status === 'transferring' && previous && now > previous.at && t.bytes >= previous.bytes ? (t.bytes - previous.bytes) * 1000 / (now - previous.at) : 0;
      if (terminal(t.status)) speedSamples.current.delete(t.id); else speedSamples.current.set(t.id, { bytes: t.bytes, at: now });
      setTransfers((old) => [{ ...t, speed }, ...old.filter((a) => a.id !== t.id)].sort((a, b) => b.created - a.created).slice(0, 50));
